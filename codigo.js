  ...
  function codigoCat(catstr) {
	...
	function listarProductos(productos) {
	  ...
	  for(nfila=0;nfila<num;nfila++) {
		...
		Accion[nfila].innerHTML= "<button id = 'eliminarBtn'>Eliminar</button>"
		Accion[nfila].firstChild.setAttribute("onclick","eliminarProductos('"+productos[nfila].id+"');");
		}
	}

function obtenerProductos() {
	  fetch('https://api-generator.retool.com/dVZqGu/productos')
            .then(res=>res.json())
            .then(data=>{
				productos=data;
				productos.forEach(
...
function eliminarProductos(iden) {
	fetch('https://api-generator.retool.com/dVZqGu/productos/'+iden, { method: "DELETE" })
